...
function canonicalPayload(payload: EatCleanCheckoutAttemptPayload) {
  return JSON.stringify({
    ...
    deliveryAddress: {
      ...
      savedAddressId: payload.deliveryAddress.savedAddressId?.trim() || '',
      label: payload.deliveryAddress.label?.trim() || '',
      placeId: payload.deliveryAddress.placeId?.trim() || '',
      latitude: Number.isFinite(payload.deliveryAddress.latitude) ? Number(payload.deliveryAddress.latitude) : null,
      longitude: Number.isFinite(payload.deliveryAddress.longitude) ? Number(payload.deliveryAddress.longitude) : null,
      ...
      entranceNote: payload.deliveryAddress.entranceNote?.trim() || '',
      deliveryMode: payload.deliveryAddress.deliveryMode || 'scheduled',
