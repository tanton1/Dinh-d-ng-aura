...
export default function StaffPerformanceBrandPanel({ periodId, isDemo = false }: { periodId: string; isDemo?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (isDemo) {
        setScore({ schemaVersion: 2, formulaVersion: 'aura-pt-performance-v1.1-2026-09-08', staffId: 'demo-trainer', staffName: 'PT Demo', periodId, assessmentRevision: 0, generatedAt: '', amountImpact: 'none', locked: false, coverage: { availableWeight: 0, totalWeight: 100, confidence: 'low', missingMetricIds: [] }, score: { value: null, provisionalValue: 0, maximum: 100, reason: 'Bản demo chỉ minh họa cấu phần Brand.' }, categories: [], gates: [], bonus: { eligibility: 'pending', recommendedAmount: null, classification: 'Chưa đủ dữ liệu', reason: 'Bản demo chưa có phiếu chấm.' }, brand: { total: 0, maximum: 10, personal: { approvedCount: 0, target: 4, score: 0, maximum: 5 }, aura: { approvedCount: 0, target: 3, score: 0, maximum: 3 }, profile: { completedCount: 0, target: 10, score: 0, maximum: 2, checklist: {} } }, evidence: { total: 0, pending: 0, approved: 0 }, automation: { sourceWarnings: [], trainerStudentCount: 0, nutritionStudentCount: 0, sessionCount: 0, projectionMetricsSkipped: false, generatedAt: '' } })
        setEvidence([])
      } else {
