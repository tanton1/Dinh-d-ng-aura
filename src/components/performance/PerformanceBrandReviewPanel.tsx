...
import { AlertTriangle, Camera, CheckCircle2, ExternalLink, Image, LoaderCircle, Megaphone, RefreshCw, ShieldCheck, UserRoundCheck, XCircle } from 'lucide-react'
import {
...
export default function PerformanceBrandReviewPanel({ isDemo = false }: { isDemo?: boolean }) {
  ...
  }, [isDemo, periodId, selected?.id, status, type])
  ...
  return <section className="performance-review" aria-label="Duyệt Thương hiệu cá nhân và Aura Brand">
    <header className="performance-review__hero"><span><UserRoundCheck /></span><div><small>AURA PERFORMANCE SCORE · BRAND 10/100</small><h2>Duyệt bằng chứng thương hiệu</h2><p>Chỉ nội dung đã đăng, đúng định vị và có bằng chứng hợp lệ mới được cộng điểm.</p></div></header>
    <div className="performance-review__toolbar">
      ...
      <label><span>Loại</span><select value={type} onChange={(event) => setType(event.target.value as typeof type)}><option value="">Tất cả Brand</option><option value="personal_content">Personal Brand</option><option value="aura_assignment">Aura Brand</option></select></label>
    ...
    <div className="performance-review__metrics"><article><small>HỒ SƠ TRONG BỘ LỌC</small><strong>{rows.length}</strong><span>{summary.submitted} chờ duyệt</span></article><article><small>PERSONAL BRAND</small><strong>{summary.personal}</strong><span>5 điểm · mục tiêu 4 asset</span></article><article><small>AURA BRAND</small><strong>{summary.aura}</strong><span>3 điểm · tối đa 3 nhiệm vụ</span></article><article><small>ĐÃ DUYỆT</small><strong>{summary.approved}</strong><span>Không ảnh hưởng tiền lương V1</span></article></div>
    ...
    {!loading && <div className="performance-review__layout">
      <div className="performance-review__queue">
        ...
        {!rows.some((item) => item.type !== 'profile_checklist') && <div className="performance-review__empty"><CheckCircle2 /><strong>Không có bằng chứng trong bộ lọc</strong><span>Hồ sơ mới sẽ xuất hiện ngay sau khi PT gửi.</span></div>}
      </div>
      <aside className="performance-review__detail">
        {selected ? <>
          <header><small>{selected.type === 'aura_assignment' ? 'AURA BRAND' : 'PERSONAL BRAND'}</small><h3>{selected.title}</h3><p>{selected.staffName} · {dateLabel(selected.postedAt)}</p></header>
        ...
        </> : <div className="performance-review__placeholder"><ShieldCheck /><strong>Chọn một bằng chứng để kiểm tra</strong><p>Đối chiếu nội dung, ngày đăng, đúng brief, bản quyền và quyền sử dụng hình ảnh khách hàng.</p></div>}
      </aside>
    </div>}
    <section className="performance-review__profile">
      <header><span><ShieldCheck /></span><div><small>PROFILE QUALITY · 2 ĐIỂM</small><h3>Checklist hồ sơ PT</h3><p>10 mục, mỗi mục đạt cộng 0,2 điểm. Checklist do quản lý/Admin xác nhận.</p></div></header>
      ...
      <footer><strong>{PERFORMANCE_PROFILE_KEYS.filter((key) => checklist[key]).length}/10 mục · {(PERFORMANCE_PROFILE_KEYS.filter((key) => checklist[key]).length * 0.2).toFixed(1).replace('.', ',')}/2 điểm</strong><button type="button" disabled={!profileStaffId || Boolean(working)} onClick={() => void saveProfile()}>{working === 'profile' ? <LoaderCircle className="spin" /> : <CheckCircle2 />} Lưu Profile Quality</button></footer>
    </section>
  </section>
}
