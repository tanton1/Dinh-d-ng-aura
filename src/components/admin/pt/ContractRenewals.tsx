...
export default function ContractRenewals({ onNavigate }: Props) {
  ...
  return <div className="renewal-page">
    ...
    {tab === 'queue' && <>
      ...
      <QueueView rows={rows} loading={loading} scope={data?.scope || 'self'} selectedCaseIds={selectedCaseIds} onToggleSelection={toggleCaseSelection} onSelectAll={(checked) => setSelectedCaseIds(checked ? new Set(rows.slice(0, 20).map((row) => row.caseId)) : new Set())} onTransfer={() => setTransferOpen(true)} onOpen={openDetail} onRenew={setRenewing} />
    ...
    {tab === 'pipeline' && <PipelineView rows={rows} loading={loading} stageCounts={data?.stats.stageCounts || null} onViewStage={selectPipelineStage} onOpen={openDetail} />}
    ...
    {selectedCaseId && <CaseInspector detail={detail} loading={detailLoading} options={data?.options || null} scope={data?.scope || 'self'} messageTemplates={messageTemplates} templateError={templateError} onClose={() => { setSelectedCaseId(null); setDetail(null) }} onReload={reloadDetailAndList} onRenew={(row) => setRenewing(row)} />}
...
function RenewalFilters(props: FilterProps) {
  const activeCount = [props.segment, props.risk, props.stage, props.sla, props.approval, props.branchId, props.assigneeId].filter((value) => value !== 'all').length
...
function QueueView({ rows, loading, scope, selectedCaseIds, onToggleSelection, onSelectAll, onTransfer, onOpen, onRenew }: {
  rows: RenewalPipelineRow[]; loading: boolean; scope: RenewalPipelineResponse['scope']; selectedCaseIds: Set<string>
...
function PipelineView({ rows, loading, stageCounts, onViewStage, onOpen }: { rows: RenewalPipelineRow[]; loading: boolean; stageCounts: Record<RenewalStage, number> | null; onViewStage: (stage: RenewalStage) => void; onOpen: (id: string) => void }) {
...
function CaseInspector({ detail, loading, options, scope, messageTemplates, templateError, onClose, onReload, onRenew }: { detail: RenewalCaseDetail | null; loading: boolean; options: RenewalPipelineResponse['options'] | null; scope: RenewalPipelineResponse['scope']; messageTemplates: RenewalMessageTemplate[]; templateError: string; onClose: () => void; onReload: () => Promise<void>; onRenew: (row: RenewalPipelineRow) => void }) {
  ...
  return <><div className="renewal-inspector-layer" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="renewal-inspector" role="dialog" aria-modal="true" aria-labelledby="renewal-inspector-title"><div className="renewal-inspector__handle" /><header><div><span>HỒ SƠ TÁI KÝ</span><h2 id="renewal-inspector-title">{row?.student.name || 'Đang tải…'}</h2><p>{row ? `${row.student.phone || 'Chưa có SĐT'} · ${row.branchName}` : 'Đang đối chiếu dữ liệu'}</p></div><button type="button" onClick={onClose}><X /></button></header>{loading || !row ? <LoadingState label="Đang tải lịch sử hồ sơ…" /> : <div className="renewal-inspector-body">
    ...
    <section className="renewal-inspector-actions"><button type="button" disabled={!row.student.phone} onClick={() => setMessageOpen(true)}><BookOpenText size={16} /> Kịch bản chăm sóc</button><button type="button" disabled={!row.student.phone} onClick={() => window.location.href = `tel:${row.student.phone}`}><Phone size={16} /> Gọi điện</button><button type="button" className="is-primary" onClick={() => onRenew(row)}>Tạo tái ký <ArrowRight size={16} /></button></section>
