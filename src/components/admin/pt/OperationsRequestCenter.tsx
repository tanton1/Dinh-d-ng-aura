  ...
  return <section className="operations-requests" aria-busy={loading}>
    ...
    <div className="operations-requests__list">
      ...
      {records.map((record) => <article key={record.id} className={`operations-request-card is-${record.status}`}>
        ...
        {record.kind === 'session' ? <div className="operations-request-card__slots"><div><small>Lịch gốc</small><strong>{slotLabel(record.originalDate, record.originalHour)}</strong></div>{record.type === 'reschedule' && <div className="is-new"><small>Lịch đề xuất</small><strong>{slotLabel(record.newDate, record.newHour)}</strong></div>}</div> : <div className="operations-request-card__slots"><div><small>Từ ngày</small><strong>{dateLabel(record.startDate)}</strong></div><div className="is-new"><small>Đến ngày</small><strong>{dateLabel(record.endDate)}</strong></div></div>}
        ...
        <div className="operations-request-card__facts">
          ...
          {record.kind === 'session' ? <><span>{record.trainerName}</span><span>{record.requestedBy === 'trainer' ? 'PT gửi' : 'Học viên gửi'}</span><span>{record.countsTowardContract ? 'Có tính buổi' : 'Miễn tính buổi'}</span></> : <><span>{record.durationDays} ngày</span><span>{record.cancelledSessionCount} ca được xử lý</span>{record.newContractEndDate && <span>Hạn mới {dateLabel(record.newContractEndDate)}</span>}</>}
        </div>
