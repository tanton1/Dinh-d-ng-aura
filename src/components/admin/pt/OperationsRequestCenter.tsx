...
export default function OperationsRequestCenter({ kind }: Props) {
  ...
  const title = kind === 'session' ? 'Đổi, hủy và thêm buổi' : 'OFF và bảo lưu'
  const subtitle = kind === 'session'
    ? 'Duyệt yêu cầu theo buổi, giữ lịch gốc, ca đề xuất, thứ tự ưu tiên và chính sách tính buổi.'
  ...
  return <section className="operations-requests" aria-busy={loading}>
    ...
    <div className="operations-requests__list">
      ...
      {records.map((record) => <article key={record.id} className={`operations-request-card is-${record.status}`}>
        <header><span className={`operations-request-card__kind is-${record.type}`}>{record.kind === 'session' ? record.type === 'additional' ? 'Đăng ký thêm ca' : record.type === 'cancel' ? 'Hủy buổi' : 'Đổi lịch' : record.type === 'off' ? 'OFF' : 'Bảo lưu'}</span><span className={`operations-request-card__status is-${record.status}`}>{statusCopy[record.status]}</span></header>
        ...
        {record.kind === 'session' ? <div className="operations-request-card__slots">{record.type !== 'additional' && <div><small>Lịch gốc</small><strong>{slotLabel(record.originalDate, record.originalHour)}</strong><span>{record.trainerName}</span></div>}{record.type !== 'cancel' && <div className="is-new"><small>{record.type === 'additional' ? 'Ca học viên đăng ký' : 'Lịch Aura gợi ý'}</small><strong>{slotLabel(record.newDate, record.newHour)}</strong><span>{record.newTrainerName || record.trainerName}{record.pairsExistingSession ? ' · ưu tiên ghép ca 1/2' : record.isPrimaryTrainer ? ' · PT chính' : ''}</span></div>}</div> : <div className="operations-request-card__slots"><div><small>Từ ngày</small><strong>{dateLabel(record.startDate)}</strong></div><div className="is-new"><small>Đến ngày</small><strong>{dateLabel(record.endDate)}</strong></div></div>}
        ...
        <div className="operations-request-card__facts">
          ...
          {record.kind === 'session' ? <><span>{record.requestedBy === 'trainer' ? 'PT gửi' : 'Học viên gửi'}</span>{record.type === 'additional' ? <><span>Ưu tiên {record.priorityTier || 3}/3</span><span>{record.weeklyScheduled || 0}/{record.weeklyTarget || 0} buổi mục tiêu tuần</span>{record.requiresManagerApproval && <span>Cần quản lý xác nhận</span>}</> : <span>{record.countsTowardContract ? 'Có tính buổi' : `Miễn tính buổi · ${record.policySequence || 1}/${record.complimentaryLimit}`}</span>}{record.suggestionRank && <span>Gợi ý #{record.suggestionRank}</span>}</> : <><span>{record.durationDays} ngày</span><span>{record.cancelledSessionCount} ca được xử lý</span>{record.newContractEndDate && <span>Hạn mới {dateLabel(record.newContractEndDate)}</span>}</>}
        </div>
