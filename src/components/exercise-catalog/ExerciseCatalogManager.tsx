...
import {
  ...
  CircleCheck, Dumbbell, Flame, Image, Images, Library, Link2, Plus, RefreshCw, Save, Search,
...
function emptyDraft(): ExerciseCatalogDraft {
  return {
    ...
    media: { startImageUrl: '', endImageUrl: '', posterUrl: '', posterImageId: '', images: [], animationUrl: '', mimeType: 'image/webp', checksum: '' },
...
function draftFromItem(item: ExerciseCatalogItem): ExerciseCatalogDraft {
  return {
    ...
    media: { ...item.media, images: item.media.images?.map((image) => ({ ...image })) || [] },
...
export default function ExerciseCatalogManager({ canPublish, isDemo = false }: { canPublish: boolean; isDemo?: boolean }) {
  ...
  const selectItem = async (item: ExerciseCatalogItem) => {
    ...
    try {
      ...
      setDirty(false); setActiveStep(0); setLastSavedAt(null); changeVersionRef.current = 0
  ...
  const closeMobileDetail = () => {
    setSelectedId(''); setPublishedItem(null); setDraft(emptyDraft()); setEditRevision(0); setDirty(false); setError(''); setNotice(''); setActiveStep(0)
  }

  const createNew = () => {
    const id = newExerciseId(); const next = emptyDraft(); next.source.sourceExerciseId = id
    setSelectedId(id); setPublishedItem(null); setDraft(next); setEditRevision(0); setDirty(true); setError(''); setNotice(''); setStepError(''); setActiveStep(0); setLastSavedAt(null); changeVersionRef.current = 1
  }
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      const matchesStatus = statusFilter === 'all' || (statusFilter === 'popular' ? popularForWomenIds.has(item.id) : statusFilter === 'working' ? item.hasWorkingDraft : item.status === statusFilter)
  ...
  return <section className={`exercise-catalog-manager ${selectedId ? 'has-mobile-detail' : ''}`} aria-label="Quản lý thư viện bài tập">
    ...
    <div className="exercise-catalog-manager__layout">
      <aside className="exercise-catalog-manager__browser"><div className="exercise-catalog-manager__tools"><label><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tên bài, nhóm cơ, dụng cụ…" aria-label="Tìm trong thư viện bài tập" /></label><button onClick={() => void loadItems()} disabled={loading} aria-label="Tải lại thư viện"><RefreshCw /></button></div><div className="exercise-catalog-manager__filters">{(['all', 'popular', 'published', 'review', 'draft', 'working'] as CatalogStatusFilter[]).map((status) => <button className={`${statusFilter === status ? 'is-active' : ''} ${status === 'popular' ? 'is-popular' : ''}`} onClick={() => setStatusFilter(status)} key={status}>{status === 'popular' && <Flame />}{statusLabels[status]}</button>)}</div><div className="exercise-catalog-manager__list">
        {filteredItems.map((item) => <button className={`${selectedId === item.id ? 'is-active' : ''} ${popularForWomenIds.has(item.id) ? 'is-popular' : ''}`} onClick={() => void selectItem(item)} key={item.id}><span className="exercise-catalog-manager__thumb">{item.media.posterUrl || item.media.startImageUrl ? <img src={item.media.posterUrl || item.media.startImageUrl} alt="" loading="lazy" /> : <Dumbbell />}</span><span><b>{item.nameVi}</b><small>{item.targetMuscles.join(' · ') || item.bodyParts.join(' · ') || 'Chưa phân nhóm'}</small><span className="exercise-catalog-manager__badges">{popularForWomenIds.has(item.id) && <em className="is-popular"><Flame />Nữ hay chọn</em>}<em className={`is-${item.hasWorkingDraft ? 'working' : item.status}`}>{item.hasWorkingDraft ? 'Đang sửa' : statusLabels[item.status]}</em></span></span>{(!item.instructionsVi.length || !item.media.startImageUrl) ? <AlertTriangle aria-label="Thiếu dữ liệu xuất bản" /> : <ChevronRight aria-label="Xem chi tiết" />}</button>)}
      ...
      <div className="exercise-catalog-manager__editor" ref={editorRef}>
        {!selectedId ? <div className="exercise-catalog-manager__empty is-large"><Dumbbell /><strong>Chọn hoặc thêm bài tập</strong><span>Biên tập khoa học theo 5 bước, không cần điền một form dài.</span></div> : <>
          ...
          <header><div><span>{publishedItem?.status === 'published' ? 'CHI TIẾT BÀI TẬP' : 'HỒ SƠ BÀI TẬP'}</span><h3>{draft.nameVi || 'Bài tập mới'}</h3><p>{autoSaving ? 'Đang tự động lưu…' : dirty ? 'Có thay đổi chưa lưu' : lastSavedAt ? `Đã lưu lúc ${lastSavedAt.toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}` : `Revision ${editRevision}`}</p></div>{popularForWomenIds.has(selectedId) ? <em className="is-popular"><Flame />Nữ hay chọn</em> : <em>{draft.status === 'review' ? 'Chờ duyệt' : 'Nháp'}</em>}</header>
          <nav className="exercise-catalog-manager__stepper" aria-label="Các bước tạo bài tập">{wizardSteps.map((step, index) => <button type="button" className={`${activeStep === index ? 'is-active' : ''} ${stepComplete(index) ? 'is-complete' : ''}`} onClick={() => { setActiveStep(index); setStepError('') }} key={step.label}><i>{stepComplete(index) ? <Check /> : index + 1}</i><span><b>{step.label}</b><small>{step.note}</small></span></button>)}</nav>
