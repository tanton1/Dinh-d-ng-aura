...
function chapterBody(chapter: CurriculumChapter) {
  ...
  return [
    ...
    '## Khung học sâu: hiểu, quan sát và tự kiểm',
    'Ba lớp dưới đây nối kiến thức của chương với dữ liệu đời sống. Hãy đọc lần lượt, trả lời câu tự kiểm bằng ví dụ của chính bạn rồi mới chuyển sang phần áp dụng.',
    '',
    ...guide.sections.flatMap((section, index) => [
      `### ${section.title}`,
      `**Hiểu đúng:** ${chapter.objectives[index]}`,
      '',
      `**Điểm neo để ra quyết định:** ${chapter.takeaways[index]}`,
      '',
    ...
    guide.workedExample,
    '',
    '## Từ kiến thức đến một quyết định có thể kiểm chứng',
    'Không cần biến mọi kiến thức trong chương thành một danh sách luật phải làm ngay. Quy trình dưới đây giúp giữ thay đổi đủ nhỏ để còn biết điều gì tạo ra kết quả.',
    '',
    ...chapter.practiceSteps.flatMap((step, index) => [
      `### ${index === 0 ? 'Quan sát hiện trạng' : index === 1 ? 'Thử một thay đổi' : 'Đánh giá lại dữ liệu'}`,
      step,
      '',
    ]),
    `**Đầu ra cần đạt:** ${chapter.practiceResult}`,
    '',
    '## Hiểu lầm thường gặp',
