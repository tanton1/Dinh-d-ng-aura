...
import { createQuote, getMySalesWorkspace, type SalesCatalog, type SalesQuoteSummary } from '../../services/ptOperationsV2Service'
...
export default function SalesPortalV2({ embedded = false }: { embedded?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const workspace = await getMySalesWorkspace()
      setQuotes(workspace.quotes); setCatalog(workspace.catalog)
      setForm((current) => ({ ...current, branchId: current.branchId || workspace.catalog.branches[0]?.id || '', packageId: current.packageId || workspace.catalog.packages[0]?.id || '' }))
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try { await createQuote(form); setForm((current) => ({ ...current, customerName: '', customerPhone: '', discount: 0, memberReferralCode: '' })); await load() }
