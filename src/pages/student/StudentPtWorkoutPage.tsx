...
function ExerciseDetail({ item, isInCurrentPlan, onBack, onOpenPlan, saved, onToggleSaved }: {
  ...
  return <article className="student-library__detail" aria-label={`Chi tiết ${item.nameVi}`}>
    ...
    <div className="student-library__detail-body">
      ...
      <section className="student-library__technique"><h3>Kỹ thuật thực hiện</h3>{item.instructionsVi.slice(0, 4).map((instruction, index) => <div className="student-library__step" key={`${item.id}-instruction-${index}`}><b>{index + 1}</b><span>{instruction}</span></div>)}</section>
      {item.breathingVi && <div className="student-library__info"><Info /><span><strong>Nhịp thở</strong>{item.breathingVi}</span></div>}
      {item.commonMistakesVi.length > 0 && <div className="student-library__warning"><AlertTriangle /><span><strong>Lưu ý an toàn</strong>{item.commonMistakesVi[0]}</span></div>}
