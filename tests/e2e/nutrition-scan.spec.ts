...
test('nutrition scan result stays responsive and does not expose Coach/PT suggestions', async ({ page }) => {
  ...
  const clarificationToggle = result.getByRole('button', { name: /Xác nhận thêm/i })
  await expect(clarificationToggle).toHaveAttribute('aria-expanded', 'false')
  await clarificationToggle.click()
  await expect(clarificationToggle).toHaveAttribute('aria-expanded', 'true')
  await expect(result.getByRole('heading', { name: 'Giúp Aura tính sát bữa ăn thực tế' })).toBeVisible()
  await expect(result.getByText('0/1 đã rõ', { exact: true })).toBeVisible()
  ...
  await expect(result.getByText('1/1 đã rõ', { exact: true })).toBeVisible()
