...
test('nutrition plan keeps the classic meal and seven-day tabs', async ({ page }) => {
  ...
  await expect(plan.getByRole('group', { name: 'Lọc nhanh thực đơn' })).toBeVisible()
  await plan.getByRole('button', { name: '1 cột' }).click()
  await expect(plan.locator('.meal-plan-live-grid')).toHaveClass(/meal-plan-live-grid--mobile-1/)
  await plan.getByRole('button', { name: '2 cột' }).click()
  await expect(plan.locator('.meal-plan-live-grid')).toHaveClass(/meal-plan-live-grid--mobile-2/)
  await plan.locator('.meal-plan-filter-trigger').click()
  await expect(plan.getByRole('combobox', { name: 'Mục tiêu dinh dưỡng' })).toBeVisible()
  await expect(plan.getByRole('combobox', { name: 'Loại bữa ăn' })).toBeVisible()
...
test('Aura Academy shows one focused course on the responsive curriculum artwork', async ({ page }) => {
  ...
  const readerTabs = page.getByRole('tablist', { name: 'Loại nội dung' })
  const pdfTab = readerTabs.getByRole('tab', { name: /^Nội Dung/ })
  if (await pdfTab.count()) {
    await expect(pdfTab).toBeVisible()
    await expect(pdfTab).toHaveAttribute('aria-selected', 'true')
  }
  await expect(readerTabs.getByRole('tab', { name: /Học cùng Aura/ })).toBeVisible()
  await readerTabs.getByRole('tab', { name: /Học cùng Aura/ }).click()
  ...
  await expect(studio.locator('.academy-learning-card')).toHaveCount(12)
  await expect(studio.getByRole('navigation', { name: 'Các bước học trong chương' }).getByRole('button')).toHaveCount(4)
  await expect(studio.getByRole('button', { name: 'Toàn văn' })).toHaveCount(0)
  await expect(studio.getByRole('button', { name: 'Đọc' })).toHaveCount(0)
