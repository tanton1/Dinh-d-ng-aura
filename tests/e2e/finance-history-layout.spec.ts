...
test.describe('Aura Finance Intelligence responsive workspace', () => {
  ...
  test('starts the operations dashboard with Aura metrics on mobile and desktop', async ({ page }) => {
    for (const width of [360, 390, 430, 1440]) {
      ...
      await payrollChart.getByRole('tab', { name: /PT Tấn/ }).click()
      await expect(payrollChart.getByRole('tab', { name: /PT Tấn/ })).toHaveAttribute('aria-selected', 'true')
      await expect(payrollChart.locator('.admin-dashboard__payroll-person strong')).toHaveText('PT Tấn')
      await expect(payrollChart.getByRole('img', { name: /Biểu đồ thu nhập PT Tấn/ })).toBeVisible()
      ...
      await expect(analytics.getByText('TỶ LỆ ĐI TẬP')).toBeVisible()
      await expect(analytics.locator('.admin-dashboard__source-counts')).toHaveCount(0)
      await expect(dashboard.locator('.admin-dashboard__attendance table')).toHaveCount(0)
      await expect(dashboard.locator('.admin-dashboard__receivables table')).toHaveCount(0)
      await expect(dashboard.locator('.admin-dashboard__attendance-metrics > span')).toHaveCount(5)
      await expect(dashboard.locator('.admin-dashboard__debt-metrics > span')).toHaveCount(3)
      const analyticsTop = await analytics.evaluate((element) => element.getBoundingClientRect().top)
      const attendanceTop = await dashboard.locator('.admin-dashboard__attendance').evaluate((element) => element.getBoundingClientRect().top)
      const payrollTop = await payrollChart.evaluate((element) => element.getBoundingClientRect().top)
      const receivablesTop = await dashboard.locator('.admin-dashboard__receivables').evaluate((element) => element.getBoundingClientRect().top)
      expect(analyticsTop).toBeLessThan(attendanceTop)
      expect(payrollTop).toBeGreaterThan(attendanceTop)
      expect(payrollTop).toBeLessThan(receivablesTop)
