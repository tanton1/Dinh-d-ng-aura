...
test('manager opportunity pool shows strict tiers and filters against learner availability on mobile', async ({ page }) => {
  ...
  await expect(officialPtSlots.locator('button.is-assigned-match').first()).toContainText('Khớp PT chính/phụ')
  ...
  await expect(officialPtSlots.locator('button.is-assigned-match').first()).toContainText('Khớp PT chính')
  await expect(officialPtSlots.locator('button.is-assigned-match').filter({ hasText: 'Vy' }).first()).toContainText('Khớp PT phụ')
  expect(await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth)).toBeLessThanOrEqual(1)
})
