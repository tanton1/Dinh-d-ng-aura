...
test('admin payroll carousel opens the actor-protected staff statement page', () => {
  ...
  assert.match(service, /firebaseScheduleOptimizerFunctions \|\| firebaseFunctions/)
  assert.match(service, /`\$\{name\}V2`/)
...
test('payroll v2 uses a blue-green asia-east1 deployment with complete lifecycle aliases', () => {
  const index = fs.readFileSync(path.join(__dirname, 'index.js'), 'utf8')
  const payrollService = fs.readFileSync(path.join(__dirname, '..', 'src', 'services', 'payrollService.ts'), 'utf8')
  assert.match(index, /const payrollOverflowOnCall/)
  assert.match(index, /region: 'asia-east1'/)
  assert.match(index, /exports\.createPayrollRunV2/)
  assert.match(index, /exports\.markPayrollRunPaidV2/)
  assert.match(index, /exports\.listStaffPayrollAttendanceV2/)
  assert.match(index, /exports\.saveWorkCalendarV2/)
  assert.match(payrollService, /firebaseScheduleOptimizerFunctions \|\| firebaseFunctions/)
  assert.match(payrollService, /`\$\{name\}V2`/)
})
