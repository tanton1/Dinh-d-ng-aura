...
test('manual receipt vouchers classify non-revenue cash and support immutable reversal', () => {
  assert.match(source, /const createManualReceiptVoucher = onCall/)
  assert.match(source, /manualReceiptJournal/)
  assert.match(source, /revenueImpact: journal\.revenueImpact/)
  assert.match(source, /purposeCode === 'receivable_collection' \? -receiptAmount : 0/)
  assert.match(source, /const reverseManualReceiptVoucher = onCall/)
  assert.match(source, /documentType: 'receipt_voucher_reversal'/)
  assert.match(source, /revenueImpact: -Number\(data\.revenueImpact \|\| 0\)/)
