...
test('dashboard and attendance queries are bounded and capability protected', () => {
  ...
  assert.match(source, /DASHBOARD_CACHE_MAX_ENTRIES = 24/)
  assert.match(source, /getOperationsDashboard = onCall\(\{[\s\S]*memory: '512MiB'[\s\S]*cpu: 1[\s\S]*maxInstances: 1[\s\S]*concurrency: 8/)
