...
test('external exercise provider remains server-only and never persists signed media urls', () => {
  ...
  assert.match(source, /secrets: YMOVE_API_KEY \? \[YMOVE_API_KEY\] : \[\]/)
...
test('catalog-only discovery does not require the external provider secret', () => {
  assert.match(source, /isExerciseCatalogDiscovery/)
  assert.match(source, /isExerciseCatalogDiscovery \? null : defineSecret\('YMOVE_API_KEY'/)
  assert.match(source, /YMOVE_API_KEY \? \[YMOVE_API_KEY\] : \[\]/)
})
...
test('exercise media player reliably plays selected videos and falls back from broken sources', () => {
  ...
  assert.match(mediaPlayerSource, /function cdnFallbackUrl/)
  assert.match(mediaPlayerSource, /onError=\{\(\) => handleMediaError\(active\)\}/)
  assert.match(mediaPlayerSource, /setFailedKeys\(\(current\) => new Set\(current\)\.add\(key\)\)/)
