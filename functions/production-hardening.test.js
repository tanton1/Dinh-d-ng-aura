...
const nutritionEntry = readFileSync(join(root, 'src', 'pages', 'student', 'NutritionPage.tsx'), 'utf8')
const nutritionController = readFileSync(join(root, 'src', 'pages', 'student', 'NutritionPageController.tsx'), 'utf8')
const nutritionCatalogFlow = readFileSync(join(root, 'src', 'pages', 'student', 'NutritionCatalogFlow.tsx'), 'utf8')
...
test('nutrition route keeps a small orchestrator and lazy task boundaries', () => {
  assert.ok(nutritionEntry.split(/\r?\n/).length < 400)
  assert.match(nutritionController, /React\.lazy\(\(\) => import\('\.\/NutritionScanFlow'\)\)/)
  assert.match(nutritionController, /React\.lazy\(\(\) => import\('\.\/NutritionCatalogFlow'\)\)/)
...
test('Aura UI rollout is Super Admin-only, fail-closed and audited', () => {
  assert.match(functionsIndex, /async function requireTrustedSuperAdmin\(request\)/)
  assert.match(functionsIndex, /hasTrustedRole\(request, actorProfile\.data\(\), new Set\(\['super_admin'\]\)\)/)
  assert.match(functionsIndex, /exports\.updateAuraUiRollout = onCall/)
  assert.match(functionsIndex, /db\.doc\('system\/ui_public_config'\)/)
  assert.match(functionsIndex, /db\.doc\(`uiRolloutAssignments\/\$\{targetUid\}`\)/)
