...
const firestoreSessionDeltaSource = readFileSync(join(repositoryRoot, 'scripts', 'firebase-firestore-session-delta.cjs'), 'utf8')
...
test('sensitive operations routes require Identity v2 capabilities in addition to legacy UI roles', () => {
  ...
  assert.match(branchScheduleWorkspaceSource, /onSnapshot\([\s\S]*?ptScheduleDrafts/)
  assert.match(branchScheduleWorkspaceSource, /accessContext\.positions\.includes\('branch_manager'\)/)
  assert.match(rules, /match \/ptScheduleDrafts\/\{draftId\}[\s\S]*?managesBranchSchedule\(resource\.data\.get\('branchId', ''\)\)[\s\S]*?allow write: if false/)
  assert.match(branchScheduleWorkspaceSource, />Lịch PT<\/button>[\s\S]*?>Cảnh báo[\s\S]*?>Học viên<\/button>/)
  assert.match(branchScheduleWorkspaceSource, /Lịch sử & khôi phục[\s\S]*?Dùng khi cần quay lại lịch đã publish/)
  assert.match(ptScheduleClientSource, /retryableReadCallables = new Set\([\s\S]*?'listPtScheduleBranches'[\s\S]*?'getPtScheduleWorkspace'/)
  assert.match(ptScheduleClientSource, /retryableCallableCodes = new Set\(\['internal', 'resource-exhausted', 'unavailable', 'deadline-exceeded'\]\)/)
  assert.match(ptScheduleClientSource, /autoRetryCallableCodes = new Set\(\['internal', 'unavailable', 'deadline-exceeded'\]\)/)
  assert.match(ptScheduleClientSource, /maximumAttempts = retryableReadCallables\.has\(name\) \? 2 : 1/)
  assert.match(ptScheduleClientSource, /retryableReadCallables\.has\(name\) \? 20_000/)
  assert.match(ptScheduleClientSource, /phase: `admin_schedule_callable_\$\{name\}`/)
  assert.match(branchScheduleWorkspaceSource, /branchCatalogState === 'error'[\s\S]*?Không thể tải phạm vi chi nhánh[\s\S]*?Kết nối lại/)
  assert.match(branchScheduleWorkspaceSource, /branchCatalogState === 'ready' && branches\.length === 0[\s\S]*?Chưa có chi nhánh hoạt động/)
...
test('staff keeps learner navigation while every work capability has a separate page', () => {
  ...
  assert.match(ptOperationsClientSource, /const maximumAttempts = readOnly \? 2 : 1/)
  assert.match(ptOperationsClientSource, /timeout: readOnly \? 20_000 : 30_000/)
  assert.match(ptOperationsClientSource, /cachedCall[\s\S]*?\{ readOnly: true \}/)
  assert.match(ptOperationsClientSource, /functions\/deadline-exceeded/)
  assert.match(ptOperationsClientSource, /staffCallableMessage\(code, rawMessage\)/)
  assert.match(ptOperationsClientSource, /Tài khoản Staff chưa đồng bộ quyền hoặc phạm vi chi nhánh/)
  assert.match(trainerPortalSource, /data-testid="staff-workspace-error"/)
  assert.match(trainerPortalSource, /Đây không phải trạng thái “chưa có chi nhánh”/)
  assert.match(trainerPortalSource, /Chi nhánh vẫn hoạt động bình thường/)
...
test('source session delta is source-read-only, create-only, exact-date linked, digest-gated, and PII-safe', () => {
  assert.match(firestoreSessionDeltaSource, /projectId: 'gen-lang-client-0246058381'/)
  assert.match(firestoreSessionDeltaSource, /databaseId: 'aura-fitness-db'/)
  assert.match(firestoreSessionDeltaSource, /projectId: 'gen-lang-client-0815966909'/)
  assert.match(firestoreSessionDeltaSource, /databaseId: 'ai-studio-aurafitnesselear-/)
  assert.match(firestoreSessionDeltaSource, /sourceReadOnly: true/)
  assert.match(firestoreSessionDeltaSource, /deletePropagation: false/)
  assert.match(firestoreSessionDeltaSource, /currentDocument: \{ exists: false \}/)
  assert.match(firestoreSessionDeltaSource, /currentDocument: \{ updateTime: item\.targetUpdateTime \}/)
  assert.match(firestoreSessionDeltaSource, /contract\.startDate <= sessionDate && contract\.endDate >= sessionDate/)
  assert.match(firestoreSessionDeltaSource, /overlap_latest_contract_start/)
  assert.match(firestoreSessionDeltaSource, /NO_CONTRACT_COVERING_DATE/)
  assert.match(firestoreSessionDeltaSource, /TARGET_SEMANTIC_DUPLICATE/)
  assert.match(firestoreSessionDeltaSource, /billingStatus: 'pending'/)
  assert.match(firestoreSessionDeltaSource, /attendanceStatus: 'pending'/)
  assert.match(firestoreSessionDeltaSource, /state\.plan\.planDigest !== approved\.planDigest/)
  assert.doesNotMatch(firestoreSessionDeltaSource, /databaseBase\(SOURCE\).*documents:commit/s)
  assert.doesNotMatch(firestoreSessionDeltaSource, /console\.(?:log|error)\([^)]*(?:studentId|sessionId|contractId|email|phone|name)/)
})
...
test('PT schedule V2 is actor-scoped, date-exact, branch-owned and command revisioned', () => {
  ...
  assert.match(ptScheduleV2Source, /const fallbackStudentMap = new Map[\s\S]*?studentWeekEligibility\(mappedContractsByStudent\.get\(studentId\) \|\| \[\], studentId, branchId, week\)\.eligible/)
  assert.match(ptScheduleV2Source, /loadLatestSubmittedFallbacks\(db, fallbackStudentMap, weeklyAvailability, week\)/)
