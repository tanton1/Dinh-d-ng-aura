...
test('module exposes the complete callable contract and keeps all writes in backend transactions', () => {
  ...
  assert.match(source, /db\.doc\(`eatCleanDeliveries\/\$\{snapshot\.id\}`\)/)
  assert.match(source, /deliveryRecordMissing: !deliveryIds\.has\(snapshot\.id\)/)
  assert.match(source, /missingDeliveryRecords/)
  ...
  assert.match(source, /mapsCallableOptions = \{ \.\.\.callableOptions, secrets: \[googleMapsApiKeySecret\] \}/)
  assert.match(source, /otpWriteOptions = \{ \.\.\.writeOptions, secrets: \[deliveryOtpSecret\] \}/)
  assert.match(source, /secrets: \[googleMapsApiKeySecret, deliveryOtpSecret\]/)
  assert.doesNotMatch(source, /bindEatCleanSecrets\s*\?/)
