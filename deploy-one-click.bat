...
echo [1/7] Branch hien tai: %CURRENT_BRANCH%
echo [1/7] Branch muc tieu : %TARGET_BRANCH%
...
if defined HAS_CHANGE (
  ...
  echo [3/7] Repo sach, san sang deploy.
)

echo [4/7] Push branch %TARGET_BRANCH% len GitHub...
...
echo [6/7] Deploy Firebase Hosting...
...
if errorlevel 1 (
  echo [ERR] Firebase deploy that bai. Kiem tra phien dang nhap Firebase.
  pause
  exit /b 1
)
