...
async function main() {
  ...
  const sessions = rows.filter((row) => row.document).map((row) => ({
  ...
  for (const session of sessions.slice(0, 2000)) {
    ...
    else if (session.billingStatus === 'charged') {
      summary.chargedPending += 1
      if (startsAt.getTime() + AUTO_CONFIRM_MS <= now.getTime()) summary.pendingOver48Hours += 1
      else summary.pendingWithin48Hours += 1
    } else if (['scheduled', 'rescheduled'].includes(session.status)) summary.unchargedDue += 1
  }
