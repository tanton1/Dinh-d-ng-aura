...
Before opening orders, the admin overview must show no order with `deliveryRecordMissing=true`. Historical orders without a delivery record require manual reconciliation because the backend cannot safely infer whether food was delivered or cancelled after dispatch.
