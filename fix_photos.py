...
# Since it's currently broken, we just replace everything from `return (` to the end.
...
new_return = '''  return (
    <div id="progress-photos-section" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {isGalleryModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: '#fff', width: '100%', maxWidth: 700, borderRadius: 24, padding: 24, maxHeight: '80vh', overflowY: 'auto' }}>
             ...
             <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 16 }}>
               {photos.map(p => (
                 <div key={p.id} style={{ position: 'relative', borderRadius: 12, overflow: 'hidden', border: '1px solid #ccc' }}>
                   ...
                   <button onClick={() => deleteUserProgressPhoto(ownerId, p.id)} style={{ position: 'absolute', top: 8, right: 8, background: '#ef4444', color: '#fff', border: 'none', borderRadius: '50%', width: 28, height: 28 }}><Trash2 size={14} /></button>
                 </div>
               ))}
             </div>
          </div>
